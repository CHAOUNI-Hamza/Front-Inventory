...
function Categoriebdc() {
  ...
  const fetchData = async () => {
    setError(null);
    try {
      const response = await axios.get('/categoriesbdc');
...
text: "Veuillez remplir tous les champs obligatoires !",
    ...
    try {
      await axios.post('/categoriesbdc', { name });
      fetchData();
      setNewServiceData({
        name: '',
      });
...
text: "Ajouté avec succès.",
  ...
  const editService = async () => {
    try {
      ...
      await axios.put(`/categoriesbdc/${id}`, { name });
      fetchData();
      Swal.fire({
        title: "Fait",
  ...
  const deleteService = async (id) => {
    try {
      ...
      if (result.isConfirmed) {
        await axios.delete(`/categoriesbdc/${id}`);
        fetchData();
        Swal.fire({
          title: "Supprimé !",

...
export default function Articles() {
      ...
      const fetchData = async () => {
        if(userInfo && userInfo.labo_id) {
          ...
          axios.get(`/laboratoires/${userInfo.labo_id}/users`)
            ...
            .catch(error => {
              console.error('There was an error fetching users laboratoire!', error);
            });
      }
      };
      fetchData();
    ...
    const handleArticlesTabClick = (tab) => {
        ...
        if (tab === 'mesArticles') {
          fetchArticles('/art');
        } else if (tab === 'tousArticlesLaboratoire') {
          fetchArticles(`/articles/laboratoire/${userInfo.labo_id}`);
        }
      };
  ...
  return (
    <div className='row'>
            ...
            <div className='col-12 col-md-6 mb-4'>
            <button className={`btn btn-art ${articlesTab === 'tousArticlesLaboratoire' ? 'active' : ''}`} 
                onClick={() => handleArticlesTabClick('tousArticlesLaboratoire')}>Tous les articles de laboratoire</button>
            </div>
            {articles.length === 0 ? (

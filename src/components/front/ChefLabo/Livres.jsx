...
export default function Articles() {
  ...
  useEffect(() => {
    ...
    const handleLivresTabClick = (tab) => {
        ...
        if (tab === 'mesLivres') {
          fetchLivres('/liv');
        } else if (tab === 'tousLivresLaboratoire') {
          fetchLivres(`/livres/laboratoire/${userInfo.labo_id}`);
        }
      };
      const editLiv = async () => {
        try {
  ...
  return (
    <div className='row'>
            ...
            <div className='col-12 col-md-6 mb-4'>
            <button className={`btn btn-art ${livresTab === 'tousLivresLaboratoire' ? 'active' : ''}`} 
                onClick={() => handleLivresTabClick('tousLivresLaboratoire')}>Tous les livres de laboratoire</button>
            </div>
            {books.length === 0 ? (
